function [] = CrankNicolsonbent()
L = 75;
tsteps = 1401; % number of time steps 
epsilon =0.1;
time_step=0.005;
...
for i=1:N
    for j=1:N
        Gauss(i+(j-1)*(N)) = sqrt(r)/(sqrt(pi)).*exp(-(r*(epsilon*xl(j)-2)^2/(2*epsilon)+(epsilon*xl(i)-tanh(2))^2/(2*epsilon)));
...
for j=1:N
for i=1:N
        kappa(:,i+(j-1)*N) = tanh(epsilon*xl(i)-tanh(epsilon*xl(j)))/sqrt(1+sech(epsilon*xl(j))^4)*AA(:,i+(j-1)*N);
end
end
kappa = invAA*kappa;
