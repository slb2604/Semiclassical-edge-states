...
for i=1:N
    for j=1:N
        Gauss(i+(j-1)*(N)) = sqrt(r)/(sqrt(pi)).*exp(-(r*(epsilon*xl(j))^2/(2*epsilon)+(epsilon*xl(i)-1)^2/(2*epsilon)));
...
kappa = r/2*(epsilon^2*kron(Position_Operator2,One)+epsilon^2*kron(One,Position_Operator2)-kron(One,One));%...
...
shading interp
alpha(s,'z')
xlim([-1.25/epsilon,1.25/epsilon])
ylim([-1.25/epsilon,1.25/epsilon])
    end
end
hold off
end